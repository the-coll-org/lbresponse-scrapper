...
def transform_shelter_row(row: dict) -> dict:
    ...
    return {
        ...
        "location_id": row.get("location_id"),
        "capacity_total": _parse_int(_get(row, "capacity_total", "capacity")) or 0,
        "population_total": _parse_int(_get(row, "population_total", "population")),
        "households_count": _parse_int(
            _get(row, "households_count", "household_count", "households")
        ),
        "women_count": _parse_int(_get(row, "women_count", "women")),
        "children_count": _parse_int(_get(row, "children_count", "children")),
        "elderly_count": _parse_int(_get(row, "elderly_count", "elderly")),
...
def transform_service_availability_row(row: dict) -> dict:
    ...
    return {
        ...
        "last_verified": _parse_datetime(_get(row, "last_verified", "last_update", "verified_at")),
    }
...
def transform_shelter_need_row(row: dict, shelter_id) -> dict:
    ...
    return {
        ...
        "people_in_need": _parse_int(_get(row, "people_in_need", "affected")),
        "description": _get(row, "description", "desc", "need_description"),
        "reported_at": _parse_datetime(_get(row, "reported_at", "report_date"))
        or datetime.utcnow(),
        "status": infer_enum_value(
            _get(row, "status"),
            ShelterNeedStatus,
...
def transform_aid_match_row(row: dict) -> dict:
    ...
    return {
        ...
        "provider_id": row.get("provider_id"),
        "quantity_provided": _parse_int(_get(row, "quantity_provided", "quantity", "amount")),
        "status": infer_enum_value(
            _get(row, "status"),
            AidMatchStatus,
            AidMatchStatus.PLANNED,
        ),
        "date": _parse_date(_get(row, "date", "match_date", "delivery_date")) or date.today(),
